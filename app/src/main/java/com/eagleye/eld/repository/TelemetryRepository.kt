    ...
    suspend fun sendTelemetry(vin: String, snapshot: VirtualDashboard.Snapshot): TelemetryResponse? {
        return try {
            ...
            val request = TelemetryRequest(
                ...
                engineHours = snapshot.totalEngineHours?.toDouble(),
                engineOdometer = snapshot.engineOdometer?.toDouble(),
                coolantTemperature = snapshot.coolantTemperature?.toDouble(),
                oilTemperature = snapshot.oilTemperature?.toDouble(),
                intakeTemperature = snapshot.intakeTemperature?.toDouble(),
                ambientTemperature = snapshot.ambientTemperature?.toDouble(),
                transmissionOilTemperature = snapshot.transmissionOilTemperature?.toDouble(),
                turboOilTemperature = snapshot.turboOilTemperature?.toDouble(),
                intercoolerTemperature = snapshot.intercoolerTemperature?.toDouble(),
                fuelTankTemperature = snapshot.fuelTankTemperature?.toDouble(),
