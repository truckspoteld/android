...
class HomeViewModel @Inject constructor(
    ...
    val driverReviewLiveData: LiveData<NetworkResult<com.eagleye.eld.models.DriverReviewResponse>>
        get() = dashboardRespository.driverReviewLiveData
...
//            }
    ...
    fun getDriverReview(context: Context) {
        viewModelScope.launch {
            if (isInternetAvailable(context)) {
                try {
                    dashboardRespository.getDriverReview()
                } catch (e: Exception) {
                    Log.e("HomeViewModel", "Network error in getDriverReview: ${e.message}", e)
                    withContext(kotlinx.coroutines.Dispatchers.Main) {
                        android.widget.Toast.makeText(context, "Network error: ${e.message}", android.widget.Toast.LENGTH_SHORT).show()
                    }
                }
