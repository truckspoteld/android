package com.truckspot.eld.utils;
...
public final class ELDGraph extends View {
    ...
    private float drawingProgress = 1.0f; // Default to 1.0 for static display
    ...
    public ELDGraph(Context context, AttributeSet attributeSet) {
        ...
        labelPaint.setColor(Color.parseColor("#1A1D26")); // home_text_main
        ...
        this.dotPaint = dp;
        dp.setColor(Color.parseColor("#2D7BFE")); // nav_icon_active
        ...
        paint.setColor(Color.parseColor("#E0E6ED")); // Light gray for subtle grid
        paint.setStrokeWidth(1.0f);

        // Configure graph plot paint
        paint2.setStyle(Paint.Style.FILL_AND_STROKE);
        paint2.setStrokeWidth(4.0f); // Slightly thicker for modern look
        paint2.setColor(Color.BLACK); 

        // Configure background paints with premium colors
        offBgPaint.setStyle(Paint.Style.FILL);
        offBgPaint.setColor(Color.parseColor("#F1F5F9")); // Very light gray-blue for OFF

        sbBgPaint.setStyle(Paint.Style.FILL);
        sbBgPaint.setColor(Color.parseColor("#EBF3FF")); // nav_active_blue for SB

        driveBgPaint.setStyle(Paint.Style.FILL);
        driveBgPaint.setColor(Color.parseColor("#E1EAFF")); // Light blue tint for DR

        onBgPaint.setStyle(Paint.Style.FILL);
        onBgPaint.setColor(Color.parseColor("#DEF7ED")); // status_on_bg for ON
    }
    ...
    public final void startAnimation() {
        ...
        animator.addUpdateListener(animation -> {
            drawingProgress = (float) animation.getAnimatedValue();
            invalidate();
        });
        animator.start();
    }
    ...
    private final void changeDutyStatus(int i, int i2, float f) {
        ...
        if (canvas2 != null) {
            float animatedMaxHour = 24f * drawingProgress;
            if (f > animatedMaxHour) return;
    ...
    private final void drawStatusLine(float f, float f2, int i) {
        ...
        if (canvas2 != null) {
            float animatedMaxHour = 24f * drawingProgress;
            if (f > animatedMaxHour) return;
            float effectiveEnd = Math.min(f2, animatedMaxHour);

            canvas2.drawLine((f * ((float) getWidth())) / ((float) this.numColumns),
                    (float) (((double) ((getHeight() * i) / this.numRows))
                            - ((((double) getHeight()) * 0.5d) / ((double) this.numRows))),
                    (effectiveEnd * ((float) getWidth())) / ((float) this.numColumns),
                    (float) (((double) ((i * getHeight()) / this.numRows))
                            - ((((double) getHeight()) * 0.5d) / ((double) this.numRows))),
                    this.graphPlotPaint);
        }
    ...
    private final void drawRowBackground(int rowIndex) {
        ...
        if (canvas2 != null) {
            ...
            float animatedMaxWidth = getWidth() * drawingProgress;

            // Draw the colored background for the entire row
            canvas2.drawRect(0.0f, top, animatedMaxWidth, bottom, backgroundPaint);
        }
    }
    ...
    private final void drawSegmentTimeLabel(float startTime, float endTime, int rowIndex) {
        ...
        float animatedMaxHour = 24f * drawingProgress;
        if (startTime > animatedMaxHour) return;
    ...
    private final void drawDot(float timeHour, int rowIndex) {
        ...
        float animatedMaxHour = 24f * drawingProgress;
        if (timeHour > animatedMaxHour) return;
